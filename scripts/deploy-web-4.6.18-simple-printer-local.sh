...
EXPECTED_WORKER_REVISION="apk-v16-web-device-v20-simple-settings-v4.6.18"
EXPECTED_SHELL_REVISION="4.6.18-apk-v16-web-device-v20-simple-settings"
EXPECTED_SW_VERSION="chefops-v4-6-18-apk-v16-web-device-v20-simple-settings"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.18 simple stable printer"
echo "=================================================="
...
grep -q '"version": "4.6.18"' package.json
...
grep -q 'LabelPrinterSettingsSimpleV20' web/src/App.jsx
grep -q 'targetAddressSpace: '\''loopback'\''' web/src/lib/device-printer-v20.js
grep -q 'stupiaks_ops.web_printer_device.v20' web/src/lib/device-printer-v20.js
grep -q 'payloadBase64: asciiBase64(stable.command)' web/src/lib/stable-label-print-v20.js
...
if grep -q 'BITMAP' web/src/lib/stable-label-print-v20.js; then
  echo "ERROR: Web stable print v20 contains BITMAP. Refusing deployment."
  exit 1
fi
if grep -q 'html-raster' web/src/lib/stable-label-print-v20.js; then
  echo "ERROR: Web stable print v20 contains HTML Raster. Refusing deployment."
  exit 1
fi
if grep -q 'Advanced Bridge\|Pairing token\|RAW Queue\|LPR queue' web/src/pages/LabelPrinterSettingsSimpleV20.jsx; then
  echo "ERROR: Complex printer controls returned to the staff page. Refusing deployment."
  exit 1
fi
...
printf '\n==================================================\n'
echo "3. Build and test Web, Worker, APK v16 freeze and connector"
echo "=================================================="
...
printf '\n==================================================\n'
echo "4. Repair/update this Mac's automatic local connector"
echo "=================================================="
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  echo "Non-macOS machine detected. The Web/Worker deployment will continue without modifying Android APK."
fi
...
printf '\n==================================================\n'
echo "6. Verify production 4.6.18 and simple Settings"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.18-$COMMIT-$attempt" || true)"
  SETTINGS_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/labels/settings?acceptance=4.6.18-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.18-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.18 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$SETTINGS_HEADERS"
  exit 1
fi
...
printf '\n==================================================\n'
echo "SUCCESS: Simple stable printer v20 deployed"
echo "=================================================="
echo "URL: $WORKER_URL/labels/settings"
echo "Commit: $COMMIT"
echo "Android APK route: frozen on accepted Stable TSPL v16"
echo "Web route: device-local IP + loopback connector permission"
echo "Staff page: Printer IP / Connect / Test label / Save only"
echo "Locked media: 40x30 mm / 203 dpi / 2 mm gap / port 9100"
echo "Shared APK profile: repaired to stable values when Save or Test label is used"
echo "Android release: not triggered and existing APK is not replaced"
