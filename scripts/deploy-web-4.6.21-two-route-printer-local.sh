...
EXPECTED_WORKER_REVISION="windows-queue-direct-ip-v23-v4.6.21"
EXPECTED_SHELL_REVISION="4.6.21-windows-queue-direct-ip-v23"
EXPECTED_SW_VERSION="chefops-v4-6-21-windows-queue-direct-ip-v23"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.21 two-route printer"
echo "=================================================="
...
grep -q 'Direct IP' web/src/pages/LabelPrinterSettingsSimpleV20.jsx
grep -q 'Kitchen Label Printer' web/src/pages/LabelPrinterSettingsSimpleV20.jsx
grep -q "mode: 'queue'" web/src/lib/device-printer-v20.js
grep -q "mode: 'raw_tcp'" web/src/lib/device-printer-v20.js
...
if grep -q 'BITMAP\|html-raster' web/src/lib/stable-label-print-v20.js; then
  echo "ERROR: Raster printing returned to the stable Web route. Refusing deployment."
  exit 1
fi
if grep -q 'Pairing token\|LPR queue' web/src/pages/LabelPrinterSettingsSimpleV20.jsx; then
  echo "ERROR: Technical controls returned to the staff page. Refusing deployment."
  exit 1
fi
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.21"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.21-$COMMIT-$attempt" || true)"
  SETTINGS_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/labels/settings?acceptance=4.6.21-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.21-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.21 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$SETTINGS_HEADERS"
  exit 1
fi
...
printf '\n==================================================\n'
echo "SUCCESS: Two-route printer settings deployed"
echo "=================================================="
echo "URL: $WORKER_URL/labels/settings"
echo "Commit: $COMMIT"
echo "Windows Web option 1: installed Printer Queue, including Kitchen Label Printer"
echo "Windows Web option 2: Direct IP RAW TCP 9100"
echo "Android APK: unchanged and frozen on Stable TSPL v16"
echo "Printer selection: stored per device + outlet and does not overwrite other devices"
