...
assert(taskLiveSource.includes("target.matches('input[type=\"file\"]')"), 'file inputs must be excluded from Task draft saving')
assert(taskLiveSource.includes("target.closest('[data-task-photo-ui]')"), 'all Task photo controls must be excluded from Task draft saving')
assert(!taskLiveSource.includes('chefops-autosave-toast'), 'Task saving must not show a global saved/saving toast')
assert(!taskLiveSource.includes("'保存中…'"), 'Task saving must not show saving text')
assert(!taskLiveSource.includes("'已保存'"), 'Task saving must not show misleading saved text')

assert(!taskLiveSource.includes('scheduleSave'), 'input/change/click must not schedule automatic saves')
assert(!taskLiveSource.includes('requestIdleCallback'), 'Task drafts must not save during browser idle time')
assert(!taskLiveSource.includes('cancelIdleCallback'), 'Task draft saving must not use idle timers')
assert(!taskLiveSource.includes('onDraftFocusOut'), 'leaving a field must not trigger a save')
assert(!taskLiveSource.includes("addEventListener('focusout'"), 'focusout must not be registered as a save trigger')
assert(!taskLiveSource.includes('setInterval('), 'Task draft saving must not run on an interval')
assert(taskLiveSource.includes("['保存进度', '完成任务']"), 'the two explicit Task actions must remain save entry points')
assert(taskLiveSource.includes('pendingCloseButton.current = closeButton'), 'closing a dirty Task must request one protective save')
assert(taskLiveSource.includes('flushDirtyDraftOnce'), 'background/pagehide may flush one dirty draft once')
assert(taskLiveSource.includes("window.addEventListener('pagehide', flushDirtyDraftOnce)"), 'pagehide must use the one-shot dirty flush')
assert(taskLiveSource.includes("document.visibilityState === 'hidden'"), 'backgrounding may use the one-shot dirty flush')
assert(!taskLiveSource.includes('if (!saveInFlight.current && changeRevision.current > savedRevision.current)'), 'realtime events must not trigger a Task save')
assert(!taskLiveSource.includes("if (activeTaskDrawer()) {\n        if (changeRevision.current > savedRevision.current)"), 'focus/foreground must not trigger a Task save')

assert(taskLiveSource.includes('photoForPreview'), 'Task photos must open a full-screen preview')
assert(taskLiveSource.includes('photoScale'), 'Task photo preview must support zoom')
assert(taskLiveSource.includes('Zoom in'), 'Task photo preview must provide zoom-in control')
assert(taskLiveSource.includes('Zoom out'), 'Task photo preview must provide zoom-out control')
assert(taskLiveSource.includes('onDoubleClick'), 'Task photo preview must support quick double-click zoom')
assert(taskLiveSource.includes('commitLocalPhotosBeforeSave'), 'explicit Task save must commit local photos first')
assert(taskLiveSource.includes('retryButtonsForLocalPhotos'), 'failed local photos must use their existing inline retry action')
assert(taskLiveSource.includes('continueExplicitSave'), 'Task save may continue only after local photos are committed')
assert(taskLiveSource.includes('localPhotoImages(activeTaskDrawer()).length'), 'Save Progress and Complete must detect local unsaved photos')
assert(taskLiveSource.includes('bypassPhotoCommit.current'), 'photo commit continuation must not loop back into the save gate')
...
console.log('TASK_PHOTO_ZOOM=true')
console.log('TASK_SAVE_COMMITS_LOCAL_PHOTOS_FIRST=true')
console.log('FAILED_PHOTO_RETRY_BEFORE_TASK_SAVE=true')
console.log('TASK_SAVE_BLOCKED_WHILE_PHOTO_UNSAVED=true')
...
console.log('TASK_SAVE_ON_REALTIME=false')
console.log('TASK_SAVE_ON_FOREGROUND=false')
