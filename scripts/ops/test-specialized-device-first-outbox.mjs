...
assert.match(outboxSource, /next_attempt_at: existing\.next_attempt_at \|\| operation\.next_attempt_at/)
assert.match(outboxSource, /const backoffActive = Number\.isFinite\(nextAttemptMs\) && nextAttemptMs > Date\.now\(\)/)
assert.match(outboxSource, /if \(!navigator\.onLine \|\| backoffActive\) return queuedResult/)
...
console.log('SPECIALIZED_RETRY_BACKOFF_SUPPRESSES_ACTIVE_RETRY_STORM=true')
