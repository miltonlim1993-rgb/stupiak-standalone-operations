...
assert.equal(validatePin('456789', '+60123456789'), '456789')
assert.throws(() => validatePin('123456', '+60123456789'), /repeated digits or 123456/)
assert.throws(() => validatePin('000000', '+60123456789'), /repeated digits or 123456/)
assert.equal(validatePassword('simple88', 'milton-owner'), 'simple88')
assert.equal(validatePassword('easy pass', 'milton-owner'), 'easy pass')
assert.throws(() => validatePassword('short7', 'milton-owner'), /at least 8 characters/)
...
const emailApprovalSource = fs.readFileSync('worker/src/email-approval-auth.js', 'utf8')
...
const runtimeEntry = fs.readFileSync('worker/src/entry.js', 'utf8')
const directoryApiSource = fs.readFileSync('worker/src/d1-directory-api.js', 'utf8')
const directoryStoreSource = fs.readFileSync('worker/src/d1-directory-store.js', 'utf8')
...
const opsControl = fs.readFileSync('web/src/pages/OpsControlApprovals.jsx', 'utf8')
...
assert.match(emailApprovalSource, /\/api\/auth\/local\/email\/start/)
assert.match(emailApprovalSource, /\/api\/auth\/local\/email\/status/)
assert.match(emailApprovalSource, /\/api\/auth\/local\/email\/setup/)
assert.match(emailApprovalSource, /\/api\/auth\/local\/email\/login/)
assert.match(emailApprovalSource, /pending_email_approval/)
assert.match(emailApprovalSource, /email_first_credential_setup/)
assert.match(emailApprovalSource, /setLocalCredential/)
assert.doesNotMatch(emailApprovalSource, /activation_code/)
...
assert.match(wrapperSource, /handleEmailApprovalAuth/)
...
assert.match(appSource, /OpsControlApprovals/)
assert.match(appSource, /<OwnerOnly><OpsControl \/><\/OwnerOnly>/)
assert.match(loginPage, /One simple login/)
assert.match(loginPage, /Request sent to Ops Control/)
assert.match(loginPage, /Create your PIN/)
assert.match(loginPage, /Save and enter OPS/)
assert.match(loginPage, /temporary Google migration fallback/i)
assert.doesNotMatch(loginPage, /Activate local login/)
assert.doesNotMatch(loginPage, /activationCode/)
assert.match(authContext, /parsed\?\.id \|\| parsed\?\.google_sub/)
assert.match(authContext, /startEmailAccess/)
assert.match(authContext, /checkPendingEmailAccess/)
assert.match(authContext, /setupPendingEmailAccess/)
assert.match(authContext, /loginEmailAccess/)
assert.match(localClient, /\/api\/auth\/local\/email\/start/)
assert.match(localClient, /\/api\/auth\/local\/email\/status/)
assert.match(localClient, /\/api\/auth\/local\/email\/setup/)
assert.match(localClient, /\/api\/auth\/local\/email\/login/)
assert.match(opsControl, /> Approve<\/Button>/)
assert.doesNotMatch(opsControl, /Approve \+ code/)
assert.doesNotMatch(opsControl, /activation_code/)
assert.doesNotMatch(opsControl, /issueActivation/)
assert.match(opsControl, /My local login/)

assert.equal(fs.existsSync('scripts/ops/apply-local-auth-migration.sh'), false)
assert.equal(fs.existsSync('scripts/ops/activate-local-auth-production.sh'), false)
assert.equal(fs.existsSync('worker/src/d1-directory-bootstrap.js'), false)
assert.equal(fs.existsSync('worker/src/d1-directory-import.js'), false)
assert.equal(fs.existsSync('worker/src/d1-directory-bootstrap-state.js'), false)
assert.equal(pkg.scripts['ops:migrate:local-auth'], undefined)
assert.equal(pkg.scripts['ops:activate:local-auth'], undefined)
assert.doesNotMatch(runtimeEntry, /handleD1DirectoryBootstrap/)
assert.doesNotMatch(runtimeEntry, /X-ChefOps-Directory-Migration-Secret/)
assert.doesNotMatch(runtimeEntry, /\/api\/internal\/d1-directory\/migrate-once/)
assert.doesNotMatch(directoryStoreSource, /LegacyDirectory|legacyDirectory|Bootstrap/)
assert.doesNotMatch(directoryStoreSource, /from '\.\/sheets\.js'/)
assert.doesNotMatch(normalDeploy, /d1 migrations apply/)
assert.doesNotMatch(normalDeploy, /migrate-once/)
...
assert.doesNotMatch(emailApprovalSource, /console\.log\([^\n]*(token|secret|password|pin)/i)
...
console.log('EMAIL_FIRST_LOGIN=true')
console.log('APPROVAL_CODE_REQUIRED=false')
console.log('FIRST_LOGIN_CREDENTIAL_SETUP=true')
console.log('SECOND_LOGIN_AFTER_SETUP=false')
console.log('STAFF_CREDENTIAL=6_digit_pin')
console.log('STAFF_PIN_COMPLEXITY=obvious_common_values_only')
console.log('MANAGEMENT_CREDENTIAL=minimum_8_character_password')
console.log('GOOGLE_LOGIN_MODE=fallback')
console.log('LOCAL_SESSION_REVOCATION=true')
console.log('DIRECTORY_RUNTIME_SOURCE=d1_only')
console.log('DIRECTORY_MIGRATION_ENDPOINT=false')
