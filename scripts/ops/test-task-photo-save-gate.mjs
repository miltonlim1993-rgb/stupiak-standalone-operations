...
assert(entrySource.includes('handlePrimaryMediaUpload'), 'canonical Worker entry must intercept file uploads before the legacy app handler')
assert(entrySource.indexOf('handlePrimaryMediaUpload(request') < entrySource.indexOf('const appResponse = await app.fetch'), 'primary media upload must run before legacy app fallback')
...
assert(!mediaUploadSource.includes('audit('), 'primary media upload acknowledgement must not wait for legacy Sheet AuditLog')
assert(mediaUploadSource.includes('文件上传失败：'), 'server must expose the file-upload failure stage instead of a generic 500')
assert(mediaRuleSource.includes('MediaRule runtime read unavailable; using built-in media policy'), 'temporary Sheet MediaRule failure must fall back to built-in photo policy')
assert(mediaRuleSource.includes('console.error'), 'MediaRule fallback must remain observable in Worker logs')
