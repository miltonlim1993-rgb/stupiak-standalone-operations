...
EXPECTED_WORKER_REVISION="compact-sop-detail-v30-v4.6.29"
EXPECTED_SHELL_REVISION="4.6.29-compact-sop-detail-v30"
EXPECTED_SW_VERSION="chefops-v4-6-29-compact-sop-detail-v30"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.29 compact SOP detail"
echo "=================================================="
...
grep -q '"version": "4.6.29"' package.json
...
grep -q 'TrainingAcknowledgement.create' web/src/pages/GuidedSopLearningV30.jsx
grep -q 'md:grid-cols-\[220px_minmax(0,1fr)\]' web/src/pages/GuidedSopLearningV30.jsx
grep -q 'fixed inset-x-0 bottom-0' web/src/pages/GuidedSopLearningV30.jsx
if grep -q 'bg-\[#f4efe3\]\|border-2 border-black\|shadow-\[4px_4px_0_#111\]' web/src/pages/GuidedSopLearningV30.jsx; then
  echo "ERROR: Poster-heavy styling returned to the SOP detail page."
  exit 1
fi
if [[ -f web/src/pages/GuidedSopLearning.jsx ]]; then
  echo "ERROR: Superseded poster SOP detail component still exists."
  exit 1
fi
...
npm ci
npm run build
bash -n scripts/deploy-web-4.6.29-compact-sop-detail-local.sh
npm run cf:render
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.29"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.29-$COMMIT-$attempt" || true)"
  TRAINING_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/training?acceptance=4.6.29-$COMMIT-$attempt" || true)"
  SOP_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/sop/acceptance?acceptance=4.6.29-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.29-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.29 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$TRAINING_HEADERS"
  printf '%s\n' "$SOP_HEADERS"
  printf '%s\n' "$DELETE_RESULT"
...
printf '\n==================================================\n'
echo "SUCCESS: Compact Ops-aligned SOP detail 4.6.29 deployed"
echo "=================================================="
echo "URL: $WORKER_URL/training"
echo "Commit: $COMMIT"
echo "Training Hub: compact Ops layout remains active"
echo "SOP detail: white/slate compact reader with thin borders"
echo "Responsive detail: Desktop three columns, Tablet two columns, Mobile one guided step"
echo "Brand yellow: limited to progress, active step and primary actions"
