...
LOG_FILE="${TMPDIR:-/tmp}/stupiaks-data-package-locked-rc-worker-${PORT}.log"
...
latest_report() {
  python3 - "$REPORT_DIR" "$OUTLET" <<'PY'
import sys
from pathlib import Path

report_dir = Path(sys.argv[1])
outlet = sys.argv[2]
files = list(report_dir.glob(f"{outlet}-prepare-media-*.json"))
if files:
    print(max(files, key=lambda path: path.stat().st_mtime_ns))
PY
}
...
if [[ -n "$SOURCE_REPORT_ARG" ]]; then
  SOURCE_REPORT="$(python3 - "$SOURCE_REPORT_ARG" <<'PY'
import sys
from pathlib import Path
...
if [[ -z "$SOURCE_REPORT" || ! -f "$SOURCE_REPORT" ]]; then
  echo "No Prepare Media report was found for $OUTLET"
  exit 1
fi
...
TEMP_VARS="$(mktemp "${TMPDIR:-/tmp}/stupiaks-data-package-locked-rc-vars.XXXXXX")"
...
echo "Locked Prepare Media source: $SOURCE_REPORT"

printf '\nStarting isolated locked Release Candidate Worker on port %s...\n' "$PORT"
(
  cd worker
...
if [[ "$READY" != "1" ]]; then
  echo "Local locked Release Candidate Worker did not become ready. Last log lines:"
  tail -n 100 "$LOG_FILE" || true
  exit 1
fi

TIMESTAMP="$(date -u +%Y-%m-%d_%H-%M-%S)"
REPORT="$REPORT_DIR/${OUTLET}-release-candidate-locked-${TIMESTAMP}.json"

printf '\nRunning locked authenticated Release Candidate verification for %s...\n' "$OUTLET"
set +e
...
printf '\nLocked Release Candidate verification finished.\n'
...
if [[ -f "$REPORT" ]]; then
  ...
  echo "No passing report was written because candidate consistency or object verification failed."
fi

if [[ "$FINAL_STATUS" -ne 0 ]]; then
  echo "Locked Release Candidate failed with exit code $FINAL_STATUS"
  exit "$FINAL_STATUS"
fi
