...
EXPECTED_WORKER_REVISION="supermarket-barcode-scanner-v25-v4.6.24"
EXPECTED_SHELL_REVISION="4.6.24-supermarket-barcode-scanner-v25"
EXPECTED_SW_VERSION="chefops-v4-6-24-supermarket-barcode-scanner-v25"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.24 supermarket barcode scanner"
echo "=================================================="
...
grep -q '"version": "4.6.24"' package.json
...
grep -q "barcodeScanner: 'supermarket-hid-live-camera-native-v25'" web/src/main.jsx
grep -q "installSupermarketBarcodeScannerV25" web/src/main.jsx
grep -q "document.addEventListener('keydown', hardwareKeydown, true)" web/src/lib/supermarket-barcode-scan-v25.js
grep -q "BarcodeDetector" web/src/lib/supermarket-barcode-scan-v25.js
grep -q "getUserMedia" web/src/lib/supermarket-barcode-scan-v25.js
grep -q "NativeBarcodeScanner" web/src/lib/supermarket-barcode-scan-v25.js
grep -q "play-services-code-scanner:16.1.0" scripts/configure-android-barcode-scanner-v25.mjs
grep -q "enableAutoZoom" scripts/configure-android-barcode-scanner-v25.mjs
...
npm ci
npm run build
bash -n scripts/deploy-web-4.6.24-supermarket-barcode-scanner-local.sh
npm run cf:render
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.24"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.24-$COMMIT-$attempt" || true)"
  LABEL_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/labels?acceptance=4.6.24-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.24-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.24 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$LABEL_HEADERS"
  exit 1
fi
...
printf '\n==================================================\n'
echo "SUCCESS: Supermarket barcode scanner 4.6.24 deployed"
echo "=================================================="
echo "URL: $WORKER_URL/labels"
echo "Commit: $COMMIT"
echo "Outlet PC: USB/Bluetooth HID scanner reads globally and submits on scanner Enter"
echo "Web camera: live BarcodeDetector scan with automatic read; photo remains fallback"
echo "Android source: native Google Code Scanner v25 prepared; no signed APK released"
echo "Android printing: existing Stable TSPL v16 release remains unchanged"
