...
EXPECTED_WORKER_REVISION="practical-bilingual-chain-tasks-v5-native-tspl-v4.6.10"
EXPECTED_SW_VERSION="chefops-v4-6-10-practical-bilingual-tasks-native-tspl-shell-v10"
...
printf '\n==================================================\n'
echo "1. Verify branch and pull the exact remote release commit"
echo "=================================================="
...
grep -q '"version": "4.6.10"' package.json
...
grep -q 'normalizeTaskBilingual' worker/src/task-bilingual-v5.js
grep -q '4.6.10-practical-bilingual-tasks' .github/workflows/android-apk.yml
...
printf '\n==================================================\n'
echo "3. Build and test Web/Worker 4.6.10"
echo "=================================================="
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.10"
echo "=================================================="
VERIFIED=""
HEADERS=""
SHELL=""
for attempt in $(seq 1 18); do
  HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.10-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.10-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but the 4.6.10 revision markers were not visible."
  printf '%s\n' "$HEADERS"
  exit 1
fi
...
printf '\n==================================================\n'
echo "6. Trigger production-signed Android 4.6.10 build"
echo "=================================================="
...
printf '\n==================================================\n'
echo "SUCCESS: Web/Worker and signed Android 4.6.10 completed"
echo "=================================================="
echo "URL: $WORKER_URL"
echo "Commit: $COMMIT"
