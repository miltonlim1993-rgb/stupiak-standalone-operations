...
EXPECTED_WORKER_REVISION="compact-training-hub-v29-v4.6.28"
EXPECTED_SHELL_REVISION="4.6.28-compact-training-hub-v29"
EXPECTED_SW_VERSION="chefops-v4-6-28-compact-training-hub-v29"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.28 compact Training Hub"
echo "=================================================="
...
grep -q '"version": "4.6.28"' package.json
...
grep -q 'bg-slate-50' web/src/pages/TrainingHubV29.jsx
grep -q 'border-slate-200' web/src/pages/TrainingHubV29.jsx
if grep -q 'bg-\[#f4efe3\]\|border-2 border-black\|shadow-\[4px_4px_0_#111\]' web/src/pages/TrainingHubV29.jsx; then
  echo "ERROR: Oversized poster styling returned to the Training Hub."
  exit 1
fi
...
npm ci
npm run build
bash -n scripts/deploy-web-4.6.28-compact-training-hub-local.sh
npm run cf:render
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.28"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.28-$COMMIT-$attempt" || true)"
  TRAINING_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/training?acceptance=4.6.28-$COMMIT-$attempt" || true)"
  SOP_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/sop/acceptance?acceptance=4.6.28-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.28-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.28 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$TRAINING_HEADERS"
  printf '%s\n' "$SOP_HEADERS"
  printf '%s\n' "$DELETE_RESULT"
...
printf '\n==================================================\n'
echo "SUCCESS: Compact Ops-aligned Training Hub 4.6.28 deployed"
echo "=================================================="
echo "URL: $WORKER_URL/training"
echo "Commit: $COMMIT"
echo "Training Hub: compact white/slate Ops layout with thin borders"
echo "Responsive grid: Desktop 3, Tablet 2, Mobile 1"
echo "Brand yellow: limited to icons, progress and primary actions"
echo "Guided SOP reader: poster-standard learning layout remains unchanged"
