...
echo
echo "=================================================="
echo "3. Build and test Web 4.6.6"
echo "=================================================="
...
echo
echo "=================================================="
echo "5. Verify production 4.6.6"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.6-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.6-$COMMIT-$attempt" || true)"
  if printf '%s' "$HEADERS" | grep -Eqi '^x-chefops-worker-revision:[[:space:]]*direct-print-media-size-fix-v4\.6\.6' \
    && printf '%s' "$SHELL" | grep -q 'chefops-v4-6-6-direct-print-media-size-fix-shell-v10'; then
    VERIFIED="yes"
    break
  fi
  sleep 5
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but the 4.6.6 revision markers were not visible."
  printf '%s\n' "$HEADERS"
  exit 1
fi
...
echo
echo "=================================================="
echo "6. Trigger production-signed Android 4.6.6 build"
echo "=================================================="
...
echo
echo "=================================================="
echo "SUCCESS: Web 4.6.6 and signed Android release completed"
echo "=================================================="
echo "URL: $WORKER_URL"
echo "Commit: $COMMIT"
echo "Worker revision: direct-print-media-size-fix-v4.6.6"
