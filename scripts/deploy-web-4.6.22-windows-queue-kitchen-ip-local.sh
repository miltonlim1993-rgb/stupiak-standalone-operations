...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.22 Windows Queue + Kitchen IP"
echo "=================================================="
...
grep -q 'pairing_token_required = \$false' web/public/print-service/stupiaks-print-service.ps1
grep -q 'schtasks.exe /Create' web/public/print-service/install-stupiaks-print-service.ps1
grep -q 'ExecutionPolicy Bypass' web/public/print-service/install-stupiaks-print-service.cmd
if grep -q 'BITMAP\|html-raster' web/src/lib/stable-label-print-v20.js web/src/lib/stable-tspl-label-v16.js; then
  echo "ERROR: Stable printing release contains Raster/BITMAP. Refusing deployment."
  exit 1
fi
...
printf '\n==================================================\n'
echo "3. Build and test Web/Worker"
echo "=================================================="
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.22 and Windows installer"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ...
  WINDOWS_SERVICE="$(curl -fsS --max-time 20 "$WORKER_URL/print-service/stupiaks-print-service.ps1?acceptance=4.6.22-$COMMIT-$attempt" || true)"
  WINDOWS_INSTALLER="$(curl -fsS --max-time 20 "$WORKER_URL/print-service/install-stupiaks-print-service.cmd?acceptance=4.6.22-$COMMIT-$attempt" || true)"
  if printf '%s' "$ROOT_HEADERS" | grep -Fqi "x-chefops-worker-revision: $EXPECTED_WORKER_REVISION" \
    ...
    && printf '%s' "$WINDOWS_SERVICE" | grep -Fq 'StupiaksRawPrinterV24' \
    && printf '%s' "$WINDOWS_INSTALLER" | grep -Fq 'ExecutionPolicy Bypass'; then
    VERIFIED="yes"
    break
  fi
  sleep 5
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.22 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$SETTINGS_HEADERS"
  exit 1
fi
...
printf '\n==================================================\n'
echo "SUCCESS: Windows Queue + Kitchen Direct IP Web release deployed"
echo "=================================================="
echo "URL: $WORKER_URL/labels/settings"
echo "Commit: $COMMIT"
echo "Windows Queue: existing FeedMe/Windows printers can be selected"
echo "Kitchen Direct IP: RAW TCP 9100 remains available"
echo "Windows service: one-click, no Node, no pairing token"
echo "Android APK unchanged; Stable TSPL native route remains frozen"
