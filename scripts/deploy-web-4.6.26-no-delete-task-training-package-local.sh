...
EXPECTED_SHELL_REVISION="4.6.26-no-delete-task-training-package-v27"
EXPECTED_SW_VERSION="chefops-v4-6-26-no-delete-task-training-package-v27"
...
printf '\n==================================================\n'
echo "1. Pull and verify 4.6.26 no-delete operational baseline"
echo "=================================================="
...
grep -q "lucide-trash-2" web/src/lib/no-delete-ui-v27.js
...
npm ci
npm run build
bash -n scripts/deploy-web-4.6.26-no-delete-task-training-package-local.sh
bash -n scripts/rebuild-clean-data-package-4.6.26-local.sh
npm run cf:render
...
printf '\n==================================================\n'
echo "5. Verify production 4.6.26"
echo "=================================================="
VERIFIED=""
for attempt in $(seq 1 18); do
  ROOT_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/?acceptance=4.6.26-$COMMIT-$attempt" || true)"
  TASK_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/tasks?acceptance=4.6.26-$COMMIT-$attempt" || true)"
  TRAINING_HEADERS="$(curl -fsSI --max-time 20 "$WORKER_URL/training?acceptance=4.6.26-$COMMIT-$attempt" || true)"
  SHELL="$(curl -fsS --max-time 20 "$WORKER_URL/sw.js?acceptance=4.6.26-$COMMIT-$attempt" || true)"
...
if [[ "$VERIFIED" != "yes" ]]; then
  echo "ERROR: Deployment completed but 4.6.26 production markers were not visible."
  printf '%s\n' "$ROOT_HEADERS"
  printf '%s\n' "$TASK_HEADERS"
  printf '%s\n' "$TRAINING_HEADERS"
  printf '%s\n' "$DELETE_RESULT"
...
printf '\n==================================================\n'
echo "SUCCESS: No-delete and corrected Task/Training shell 4.6.26 deployed"
echo "=================================================="
echo "URL: $WORKER_URL"
echo "Commit: $COMMIT"
echo "Hard DELETE: blocked by Worker with HTTP 405"
echo "Web: Trash/Delete controls removed"
echo "Future Android build: same no-delete UI source included"
echo "Tasks: restored v4/v5 bilingual baseline"
echo "Training: valid station SOPs retained; duplicate SOP-OPS-101..108 excluded"
