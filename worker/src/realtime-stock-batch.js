...
async function replayResult(db, mutationId) {
  const row = await db.prepare('SELECT result_json FROM ops_mutations WHERE mutation_id = ? LIMIT 1')
...
async function markQueued(db, mutationIds) {
  ...
  const marks = mutationIds.map(() => '?').join(',')
  await db.prepare(`
    UPDATE sheet_sync_outbox
    SET status = 'queued', queued_at = ?, last_error = ''
    WHERE mutation_id IN (${marks})
  `).bind(now(), ...mutationIds).run()
}
...
async function enqueueMirrors(env, messages) {
  ...
  try {
    if (typeof env.SHEET_SYNC_QUEUE.sendBatch === 'function') {
      await env.SHEET_SYNC_QUEUE.sendBatch(messages.map((body) => ({ body })))
    } else if (typeof env.SHEET_SYNC_QUEUE.send === 'function') {
      await Promise.all(messages.map((body) => env.SHEET_SYNC_QUEUE.send(body)))
    } else {
      return false
    }
...
async function saveAtomicBatch(request, env) {
  ...
  const suppliedMutationId = String(body.mutation_id || request.headers.get('X-ChefOps-Mutation-Id') || '').trim()
  const baseMutationId = (suppliedMutationId || `stock-batch:${await digest(JSON.stringify({ outletId, countDate, items: normalizedForId }))}`)
    .slice(0, 150)
  ...
  const d1Rows = (d1Query.results || []).map(realtimeRecord)
  const allCounts = mergeRows(sheetCounts, d1Rows)
  ...
  for (const input of rawItems) {
    ...
    sameDateByAlias.set(`list:${stockList.stock_list_id}`, { ...record, __realtime: { version } })
  }
  ...
  const recordSql = `
    ...
    ) VALUES ${placeholders(commits.length, 12)}
  ...
  `
  const recordBindings = commits.flatMap((item) => [
    'StockCount', item.id, outletId, countDate, 'counted', JSON.stringify(item.record),
    item.version, item.createdAt, item.createdBy, timestamp, user.email, '',
  ])
  ...
  const mutationSql = `
    ...
    ) VALUES ${placeholders(mutationRows.length, 10)}
  `
  const requestedAt = String(body.requested_at || timestamp)
  const mutationBindings = mutationRows.flatMap((item) => [
    item.mutationId,
    outletId,
    item.entity,
    item.entityId,
  ...
  const outboxSql = `
    ...
    ) VALUES ${placeholders(commits.length, 9)}
  `
  const outboxBindings = commits.flatMap((item) => [
    item.mutationId,
    'StockCount',
    item.id,
    outletId,
  ...
  try {
    await env.OPS_DB.batch([
      env.OPS_DB.prepare(recordSql).bind(...recordBindings),
      env.OPS_DB.prepare(mutationSql).bind(...mutationBindings),
      env.OPS_DB.prepare(outboxSql).bind(...outboxBindings),
    ])
  } catch (error) {
...
export async function handleAtomicStockCountBatch(request, env, url) {
