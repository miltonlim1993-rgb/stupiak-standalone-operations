...
function normalizeResponses(config, input) {
  ...
  for (const row of Array.isArray(input) ? input : []) {
    ...
    result[itemId] = cleanResponse(row)
  }
  return result
}

export function mergeTaskResponsePatches(config, currentResponses = {}, input = []) {
  const allowed = new Set(items(config).map((item) => String(item.id)))
  const result = { ...(currentResponses || {}) }
  for (const row of Array.isArray(input) ? input : []) {
    const itemId = String(row?.item_id || '')
    if (!allowed.has(itemId)) continue
    const previous = result[itemId] || { value: '', remark: '', corrective_action: '' }
    result[itemId] = {
      value: Object.prototype.hasOwnProperty.call(row, 'value') ? (row.value ?? '') : (previous.value ?? ''),
      remark: Object.prototype.hasOwnProperty.call(row, 'remark')
        ? String(row.remark || '').slice(0, 1000)
        : String(previous.remark || '').slice(0, 1000),
      corrective_action: Object.prototype.hasOwnProperty.call(row, 'corrective_action')
        ? String(row.corrective_action || '').slice(0, 2000)
        : String(previous.corrective_action || '').slice(0, 2000),
    }
  }
  return result
}
...
function validateCompletion(config, state, photos, dateText) {
  ...
  for (const item of items(config)) {
    ...
    const itemResult = evaluate(item, response)
    if (item.required && itemResult === 'incomplete') missingItems.push(item.name)
    if (itemResult === 'fail' && item.corrective_action_on_fail && !String(response?.corrective_action || '').trim()) {
...
function applyClientDraft(state, config, body) {
  const next = { ...state, responses: { ...(state.responses || {}) } }
  if (Array.isArray(body.response_patches)) {
    next.responses = mergeTaskResponsePatches(config, next.responses, body.response_patches)
  } else if (Array.isArray(body.responses)) {
    next.responses = normalizeResponses(config, body.responses)
  }

  if (Object.prototype.hasOwnProperty.call(body, 'completion_notes_patch')) {
    next.completion_notes = String(body.completion_notes_patch || '').slice(0, 3000)
  } else if (body.completion_notes !== undefined) {
    next.completion_notes = String(body.completion_notes || '').slice(0, 3000)
  }
  return next
}
...
export async function handleD1OperationalTaskAction(request, env, url) {
  ...
  try {
    ...
    const taskMutationId = mutationId(request, body, action)
    const mayRetryConflict = !Array.isArray(body.responses)
    let result = null
    let finalState = null

    for (let attempt = 0; attempt < (mayRetryConflict ? 2 : 1); attempt += 1) {
      ...
      const state = applyClientDraft(parseState(task), config, body)
      ...
      try {
        result = await mutate(request, env, {
        ...
        if (attempt > 0 || !mayRetryConflict || mutationError?.code !== 'realtime_version_conflict') throw mutationError
        const latest = await d1Record(env, 'Task', task.id)
        if (!latest) throw mutationError
        task = latest
      }
    }

    if (!result) {
      const error = new Error('Task save did not commit')
      error.status = 503
      error.code = 'task_save_uncommitted'
      throw error
    }
    ...
    return json(request, env, {
      ...
      storage: 'd1',
      sheet_read: false,
      merge_mode: Array.isArray(body.response_patches) ? 'field-patch' : 'legacy-full',
    })
  } catch (error) {
