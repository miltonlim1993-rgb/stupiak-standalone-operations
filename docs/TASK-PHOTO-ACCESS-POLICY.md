...
The retired template is set to `is_active = FALSE` in `Stupiak's Ops Master > TaskTemplates`. Historical `Task` records are not deleted or rewritten. Runtime bootstrap applies the same policy so cached or previously created duplicate tasks do not appear beside the retained task.
...
> 同类物品请放在同一张照片一起拍摄，不要逐件分开拍；每组最多可上传 10 张。
>
> Photograph matching items together in one frame instead of taking separate photos for each item; each group supports up to 10 photos.
...
- Old outlet/device rules of 8 or 4 photos are normalized to exactly 10 for Task and Urgent Issue modules.
...
Both normal and prefixed role values are supported, for example `staff` and `role_staff`.
...
6. Sign in as `staff`, `role_staff`, `leader` and `supervisor`; Tasks, Training and SOP must open.
7. Attempt `/ops-control` as those roles; the app must redirect to `/tasks`.
8. Sign in as `manager`, `role_manager` or `owner`; `/ops-control` must remain available.
